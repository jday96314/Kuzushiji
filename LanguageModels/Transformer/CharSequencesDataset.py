...
def ObjectDetectorLabelsToParallelVectors(object_detections_str, sorted_charset, image_width, image_height):
	...
	for unicode_val, x, y in labels:
		...
		# The 2 is added because 0 and 1 are reserved for sos, eos, and pad.
		char_type = np.array(np.argwhere(sorted_charset == unicode_val)[0] + 3) 
...
class FaceLandmarksDataset(Dataset):
	def __init__(
		...
		#char_label_dim_count = len(sorted_charset) + 2
		# self.SOS_char_label = np.zeros(char_label_dim_count)
		# self.SOS_char_label[-2] = 1
		self.SOS_char_label = [0]
		self.SOS_location = [1, 0] # Top right

		# self.EOS_char_label = np.zeros(char_label_dim_count)
		# self.EOS_char_label[-1] = 1
	...
	def __getitem__(self, image_index):
		...
		mangled_location_sequence, mangled_label_sequence = self.DataMangler.GetMangledSequence(
			ground_truth_location_sequence, ground_truth_label_sequence)
		return (
